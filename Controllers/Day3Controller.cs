...
namespace WebApplication2022_NetCore8_MVC.Controllers
{
    public class Day3Controller : Controller
    {
        ...
        public Day3Controller(ILogger<Day3Controller> logger, MvcUserDbContext context)
        {  //                                                                                    ****************************（自己動手加上）
            _logger = logger;
            _db = context;    //*****************************（自己動手加上）
     ...
     */
        ...
        */
        ...
        public ActionResult EditConfirm([Bind("UserId, UserName, UserSex, UserBirthDay, UserMobilePhone")] UserTable _userTable)
        {
        ...
        //[ValidateAntiForgeryToken]   // 避免XSS、CSRF攻擊，需搭配 List檢視畫面下方的輸入表格
        public ActionResult Search2(string _ID)   // 重點(2)！！因為目前的路由設定，只能接受 id這個變數 （需要修改 Program.cs的  Route設定。把 id 改 _ID 才行）
        {
